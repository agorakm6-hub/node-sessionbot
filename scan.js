...
// ====== ВЕРСИЯ GEMINI (БЕСПЛАТНАЯ 1.5 FLASH) ======
...
1. Тему письма (Subject) на английском. Например: "Report of Personal Data Leak on Telegram Channel"

2. Шаблон жалобы в поддержку Telegram на английском языке. ВАЖНО:
   - НЕ добавляй "[Your Name/Organization]" в конце
   - НЕ используй русские слова ("probiv", "пробив")
   - Опиши КОНКРЕТНОЕ нарушение (что публикуется)
   - Укажи ссылку на канал (${channelLink})
   - Укажи ссылку на сообщение (${violationLink})

3. Список законов Европейского Союза (максимум 3 ссылки), которые нарушены.
