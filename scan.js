...
const CHUNK_SIZE = 1; // по одному сообщению — тщательнее, но медленнее (см. троттлинг ниже)
...
${listText}`;
    ...
    if (isBlocked) {
        // Раньше вся пачка терялась целиком из-за одного проблемного сообщения —
        // теперь делим пополам и анализируем половины отдельно, чтобы не терять
        // остальные (нормальные) сообщения из-за одного тяжёлого случая.
        if (messages.length === 1) {
            // Дошли до одного сообщения, и Gemini всё равно отказывается его
            // анализировать — сам этот отказ является сильным сигналом:
            // такое случается почти всегда только на крайне тяжёлом контенте
            // (в первую очередь CSAM), который Google блокирует безусловно,
            // без возможности ослабить настройками safetySettings.
            return [{
                id: messages[0].id,
...
// ====== АДМИНЫ ГРУППЫ (для фильтрации — Telegram банит группы по постам админов, не обычных участников) ======
...
// ====== ОСНОВНОЙ АНАЛИЗ (канал или группа — определяется автоматически по типу сущности) ======
async function scanChannel(chatId, username, progressMsgId, mode) {
    ...
    // Приоритет — твой явный выбор кнопкой. Автоопределение по данным Telegram —
    // только запасной вариант, если кнопку не нажимали (например, ссылку прислали сразу).
