...
const CHUNK_SIZE = parseInt(process.env.SCAN_CHUNK_SIZE || '20', 10);
...
async function goToStartScreen(chatId) {
    ...
    await showScreen(chatId, startMenuText(), startMenuKeyboard());
}
...
function cooldownText(remainingMs) {
    ...
    return `⏳ Кулдаун ещё ${mins} мин. Бот общий — квота нейросети одна на всех, это защищает от перегрузки и банов ключей.`;
}
...
// админов и счётчик заявок на подтверждение, под-админу — только бан/разбан.
...
async function scanChannel(chatId, link, mode) {
    ...
    if (link.type === 'invite') {
        await showScreen(chatId, '🔐 Приватная ссылка — пытаюсь получить доступ к каналу/группе...', stopKeyboard);
    ...
    if (isGroupChat) {
        await showScreen(chatId, '👥 Это группа — собираю сообщения админов и создателя (проверяем только их — за обычных участников группу не банят)...', stopKeyboard);
        ...
        if (adminIds.size) {
            ...
            if (adminMessages) {
                textMessages = adminMessages;
            } else {
                await showScreen(chatId, '⚠️ Не удалось выделить именно сообщения админов — проверяю всех участников группы (может быть менее точно).', stopKeyboard);
            }
        } else {
            await showScreen(chatId, '⚠️ Не удалось определить список админов группы — проверяю всех участников (может быть менее точно).', stopKeyboard);
        }
    }

    if (!textMessages.length) {
        await showScreen(chatId,
            isGroupChat ? 'Не нашлось текстовых сообщений от админов/создателя группы для проверки.' : 'В канале не нашлось текстовых сообщений для проверки.',
            { inline_keyboard: [[menuButton()]] });
        return;
    }
    ...
    if (!violations.length) {
        let text = `${abortedNote}✅ Проверено ${textMessages.length} сообщений в ${channelLink} — явных нарушений не найдено.\n⏱ Заняло: ${formatElapsed(Date.now() - scanStartedAt)}`;
    ...
    awaitingSelection.set(chatId, { channelLink, violations, isGroupChat, username, textMessagesCount: textMessages.length });
    await showScreen(chatId,
...
function startMenuKeyboard() {
    return { inline_keyboard: [[btn('📡 Канал', 'mode_channel', 'primary'), btn('👥 Группа', 'mode_group', 'primary')]] };
}
...
bot.on('callback_query', async (query) => {
    ...
    if (data === 'mode_channel' || data === 'mode_group') {
        ...
        if (remaining > 0) {
            await bot.answerCallbackQuery(query.id, { text: cooldownText(remaining), show_alert: true });
            return;
        }
        await bot.answerCallbackQuery(query.id);
...
bot.on('message', async (msg) => {
    ...
    const remaining = checkCooldown(chatId);
    if (remaining > 0) {
        await showScreen(chatId, `⏳ <b>Кулдаун ещё активен</b>\n\n${cooldownText(remaining)}`, { inline_keyboard: [[menuButton()]] });
        return;
    }
