...
//  v4: законы через AI-поиск, выбор нарушений для репорта,
//      правильные ссылки на приватные каналы, честный кулдаун,
//      редактируемое меню, админ-панель с баном
// ============================================================
...
// Запасной список законов — используется только если AI-поиск законов не сработал
// (например нет доступа к google_search grounding у ключа/модели).
...
// ====== АНТИ-СЛИП (бесплатный Render засыпает без входящего трафика — сами себя пингуем) ======
...
async function analyzeChunk(messages, tosText, attempt = 1, triedKeys = new Set()) {
    // На 429 (квота конкретного ключа кончилась) гарантированно перебираем ВСЕ ключи из пула,
    // прежде чем сдаться.
    const MAX_ATTEMPTS = Math.max(4, GEMINI_API_KEYS.length + 1);
...
- Слово или часть URL/домена, которые ВЫГЛЯДЯТ похожими на что-то запрещённое (например часть ссылки похожа на аббревиатуру) — НЕ являются доказательством сами по себе. Нужен реальный контент — описание, предложение, ссылка с явным содержанием, а не просто совпадение букв.
...
${listText}`;
    ...
    if (!res.ok) {
        ...
        const allKeysTried = triedKeys.size >= GEMINI_API_KEYS.length;

        if (isQuota && !allKeysTried && attempt < MAX_ATTEMPTS) {
            console.error(`Gemini 429 у ключа ...${key.slice(-6)} (попытка ${attempt}/${MAX_ATTEMPTS}), переключаюсь на другой ключ`);
            return analyzeChunk(messages, tosText, attempt + 1, triedKeys);
        }
        if ((isQuota || isServerError) && attempt < MAX_ATTEMPTS) {
...
// ====== ЗАПАСНЫЕ ШАБЛОНЫ РЕПОРТА (если AI-генерация текста репорта не сработала) ======
...
// Раньше был захардкоженный список ссылок на законы. Теперь Gemini сам ищет
// реальные действующие ссылки под конкретное нарушение через грaундинг в Google.
...
- Без пояснений и вступлений, только список.`;
    ...
    // Приоритет — ссылки из grounding-метаданных: их реально проверил поиск.
    ...
    // Фолбэк — вытащить URL прямо из текста ответа.
...
// ====== ССЫЛКА НА КАНАЛ/ГРУППУ (правильный формат и для публичных, и для приватных) ======
// Публичные — https://t.me/username/msgId
// Приватные (без юзернейма) — https://t.me/c/<internal_id>/msgId — только так
// ссылка на конкретное сообщение открывается корректно в приватном канале.
...
const awaitingSelection = new Map(); // chatId -> { channelLink, violations, isGroupChat, username, textMessagesCount }
const lastMenuMsgId = new Map();     // chatId -> message_id последнего "менюшного" сообщения (редактируем его, а не шлём новое)
const lastScanByUser = new Map();    // chatId -> timestamp последнего РЕАЛЬНОГО скана (для кулдауна)
const usersMap = new Map();          // chatId -> { username, firstName, banned, lastSeen }

// ====== МЕНЮ: редактируем существующее сообщение вместо отправки нового ======
async function showMenu(chatId, text, keyboard) {
    const existing = lastMenuMsgId.get(chatId);
    if (existing) {
        try {
            await bot.editMessageText(text, { chat_id: chatId, message_id: existing, parse_mode: 'HTML', reply_markup: keyboard });
            return;
        } catch (e) {
            // сообщение недоступно для редактирования (удалено/слишком старое) — пошлём новое
        }
    }
    const sent = await bot.sendMessage(chatId, text, { parse_mode: 'HTML', reply_markup: keyboard });
    lastMenuMsgId.set(chatId, sent.message_id);
}
...
async function showCooldownNotice(chatId, remainingMs) {
    const mins = Math.ceil(remainingMs / 60000);
    await showMenu(chatId,
        `⏳ <b>Действует кулдаун</b>\n\n` +
        `Подожди ещё <b>${mins} мин</b> перед следующей проверкой.\n\n` +
        `Причина: бот общий для всех пользователей, а квота нейросети — одна на всех. Кулдаун защищает от перегрузки и банов ключей.`,
        { inline_keyboard: [[{ text: '◀️ В меню', callback_data: 'back_to_start' }]] });
}
...
async function sendBannedNotice(chatId) {
    await bot.sendMessage(chatId, '🚫 Ты заблокирован в этом боте.');
}
...
function renderAdminMenu() {
    ...
    text += list.length ? '\n\nНажми на пользователя, чтобы забанить/разбанить:' : '\n\nПока никто не писал боту.';

    const rows = list.map(([id, u]) => {
        const label = u.username ? `@${u.username}` : (u.firstName || `id${id}`);
        const status = u.banned ? '🚫' : '✅';
        return [{ text: `${status} ${label} (${id})`, callback_data: `admin_toggle_${id}` }];
    });
    rows.push([{ text: '◀️ В меню', callback_data: 'back_to_start' }]);
    return { text, keyboard: { inline_keyboard: rows } };
}

// ====== ВЫБОР НАРУШЕНИЙ ДЛЯ РЕПОРТА (несколько сразу — через запятую, или "все") ======
function parseSelection(text, max) {
    const raw = text.trim().toLowerCase();
    if (raw === 'все' || raw === 'всё' || raw === 'all') {
...
async function handleSelectionReply(chatId, text) {
    ...
    if (!indices) {
        await bot.sendMessage(chatId, `❌ Не понял номера. Напиши числа через запятую в диапазоне 1-${state.violations.length} (например: 1,3,5) или слово "все".`);
        return; // остаёмся в режиме ожидания, пусть попробует ещё раз
    }
    awaitingSelection.delete(chatId);

    const genMsg = await bot.sendMessage(chatId,
        `🤖 <b>Готовлю ${indices.length > 1 ? 'репорты' : 'репорт'}</b> по выбранным нарушениям (${indices.join(', ')})...`,
        { parse_mode: 'HTML' });
    ...
    for (const idx of indices) {
        ...
        await bot.sendMessage(chatId, `📋 <b>Репорт #${idx}</b> — ${escapeHtml(v.category_ru || '—')} (${v.severity || '?'}/5)\nПост: ${vLink}`, { parse_mode: 'HTML' });
        await bot.sendMessage(chatId, `<pre>${escapeHtml(reportText)}</pre>`, { parse_mode: 'HTML' });
        await bot.sendMessage(chatId, `⚖️ <b>Законы к нарушению #${idx}</b>:`, { parse_mode: 'HTML' });
...
// Возвращает { pending: true } если это была просто отправка заявки на вступление
// (реального скана не было — кулдаун за это не считается).
async function scanChannel(chatId, link, progressMsgId, mode) {
    ...
    if (link.type === 'invite') {
        ...
        entity = await client.getEntity(link.username); // бросит ошибку, если канал не существует
    }
    ...
    await bot.sendMessage(chatId,
        ...
        `Напиши номера через запятую, например <code>1,3,5</code>, или слово <code>все</code> — можно выбрать сразу несколько.`,
        { parse_mode: 'HTML', reply_markup: { inline_keyboard: [[{ text: '◀️ Отмена', callback_data: 'cancel_selection' }]] } });
}

// ====== СТАРТ-МЕНЮ (текст упрощён) ======
function startMenuText() {
    return `🛡 <b>Channel Guard</b> — проверка каналов и групп Telegram на нарушения.\n\n` +
        ...
        `🔁 Кулдаун между проверками: ${Math.round(SCAN_COOLDOWN_MS / 60000)} мин (общая квота на всех).`;
}
...
bot.onText(/^\/start$/, async (msg) => {
    ...
    if (isBanned(chatId)) { await sendBannedNotice(chatId); return; }
    awaitingLink.delete(chatId);
    awaitingSelection.delete(chatId);
    await showMenu(chatId, startMenuText(), startMenuKeyboard());
});

bot.onText(/^\/admin$/, async (msg) => {
    const chatId = msg.chat.id;
    if (msg.from.id !== ADMIN_ID) return; // тихо игнорируем чужих
...
bot.on('callback_query', async (query) => {
    ...
    if (isBanned(chatId) && !data.startsWith('admin_')) { await sendBannedNotice(chatId); return; }

    if (data.startsWith('admin_toggle_')) {
        if (query.from.id !== ADMIN_ID) return;
        const targetId = Number(data.slice('admin_toggle_'.length));
...
bot.on('message', async (msg) => {
    ...
    touchUser(msg.from, chatId);
    if (isBanned(chatId)) { await sendBannedNotice(chatId); return; }
    if (msg.text.startsWith('/')) return; // команды обрабатываются через onText выше

    // Ждём номера нарушений для репорта — не ссылку
    ...
    if (remaining > 0) {
        await showCooldownNotice(chatId, remaining);
        return; // кулдаун ещё активен — не сбрасываем и не трогаем
    }

    const chosenMode = awaitingLink.get(chatId)?.mode || null;
    awaitingLink.delete(chatId);

    lastScanByUser.set(chatId, Date.now()); // ставим кулдаун ДО скана, чтобы не спамили повторными кликами
    const wait = await bot.sendMessage(chatId, '🔌 <b>Подключаюсь...</b>', { parse_mode: 'HTML' });
    try {
        const result = await scanChannel(chatId, link, wait.message_id, chosenMode);
        if (result && result.pending) {
            lastScanByUser.delete(chatId); // заявка на вступление — это не полноценный скан, кулдаун не считаем
        }
    } catch (e) {
        console.error('scanChannel error:', e.message);
        lastScanByUser.delete(chatId); // ошибка (например неверная/несуществующая ссылка) — кулдаун не считаем
