...
// Ссылки на законы по типу нарушения — подбираются автоматически под конкретный случай.
// ⚠️ Это ориентировочная подборка типовых актов EU по теме, не юридическая
// консультация — перед подачей формальной жалобы стоит перепроверить.
const LAW_MAP = {
    personal_data: [
    ...
    illegal_goods: [
        'https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX:32022R2065' // Digital Services Act — illegal content
    ],
    csam: [
        'https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX:32011L0093' // Directive on combating sexual abuse of children
    ],
    terrorism_violence: [
        'https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX:32021R0784' // Terrorist Content Online Regulation
    ],
    hate_speech: [
        'https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX:32008F0913' // Framework Decision on racism and xenophobia
    ],
    copyright: [
        'https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX:32019L0790' // Copyright in the Digital Single Market Directive
    ],
    malware_fraud: [
        'https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX:32022R2065' // DSA — illegal content covers scams/malware too
    ],
    other: [
        'https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX:32022R2065' // общий акт EU про незаконный контент онлайн
    ]
};
...
async function analyzeChunk(messages, attempt = 1) {
...
ВАЖНО: сообщения с пометкой [ПЕРЕСЛАНО] — это репост из другого источника, а не оригинальный текст владельца канала. Если пересланное сообщение выглядит как официальное уведомление от Telegram/поддержки Telegram (например про безопасность аккаунта, правила, обновления) — это НЕ фишинг и НЕ выдача себя за другое лицо, канал просто переслал чужой пост. Оценивай такие сообщения нейтрально, если сам канал не добавил к ним собственный обманный контент.

Проверь список сообщений ниже. Верни ТОЛЬКО JSON-массив (без markdown, без пояснений) вида:
[{"id": 123, "category_ru": "тип нарушения кратко на русском", "category_en": "короткая фраза на английском, описывающая суть нарушения в стиле юридической жалобы, например 'the publication of leaked personal data without consent' — без упоминания названия или имени канала внутри этой фразы", "law_type": "personal_data|illegal_goods|csam|terrorism_violence|hate_speech|copyright|malware_fraud|other", "severity": 1-5, "reason": "краткая причина на русском"}]
...
${listText}`;
    ...
    const res = await fetch(GEMINI_URL, {
        ...
        body: JSON.stringify({
            ...
            // Мы просим модель ПРОЧИТАТЬ и классифицировать уже существующий текст
            // ради модерации, а не сгенерировать вредный контент — поэтому ослабляем
            // фильтры, иначе Gemini иногда блокирует свой же ответ на канал, где
            // реально есть тяжёлые нарушения, и мы теряем именно то, что искали.
            safetySettings: [
    ...
    if (!res.ok) {
        ...
        if (transient && attempt < MAX_ATTEMPTS) {
            ...
            console.error(`Gemini ${res.status} (попытка ${attempt}/${MAX_ATTEMPTS}), жду ${waitMs}мс и пробую снова`);
            await new Promise((r) => setTimeout(r, waitMs));
            return analyzeChunk(messages, attempt + 1);
        }
        throw new Error(`Gemini ${res.status}: ${errText.slice(0, 300)}`);
    }
    const data = await res.json();

    // Если Gemini заблокировал ответ целиком — это не значит "нарушений нет",
    // это значит "не смог проверить этот кусок", и это должно быть видно,
    // а не тихо превращаться в пустой результат.
    if (data.promptFeedback?.blockReason) {
        throw new Error(`Gemini заблокировал проверку этой пачки сообщений (${data.promptFeedback.blockReason}) — возможно, там что-то серьёзное, стоит проверить вручную.`);
    }
    const candidate = data.candidates?.[0];
    if (candidate?.finishReason === 'SAFETY' || candidate?.finishReason === 'PROHIBITED_CONTENT') {
        throw new Error(`Gemini заблокировал ответ по этой пачке сообщений (${candidate.finishReason}) — возможно, там что-то серьёзное, стоит проверить вручную.`);
    }
...
// ====== ПРОГРЕСС-БАР (классический терминальный, моноширинные блоки — это
// максимум графики, доступный в обычном тексте Telegram без Mini App;
// плавную заливку цветом текстовое сообщение отрендерить физически не может) ======
function renderProgressBar(current, total, width = 20) {
    ...
    const partials = ['', '▒', '▓']; // промежуточные ступени для более плавного вида
    const partialChar = partial > 0.66 ? partials[2] : partial > 0.33 ? partials[1] : '';
...
// ====== ЗАПАСНЫЕ ШАБЛОНЫ (если у нейросети в этот момент кончился лимит) ======
const COMPLAINT_TEMPLATES = [
    (channelLink, violationLink, desc) => `Dear Telegram Support Team,

I am writing to report a serious violation of Telegram's Terms of Service on the channel ${channelLink}. Specifically, the channel is engaged in ${desc}, as evidenced by the post ${violationLink}.
...
I would like to report the channel ${channelLink} for violating Telegram's Terms of Service. The post ${violationLink} clearly demonstrates ${desc}.
...
This is a formal report regarding the channel ${channelLink}, which is in breach of Telegram's Terms of Service through ${desc}. A clear example of this can be found at ${violationLink}.
...
// ====== ЖАЛОБА, НАПИСАННАЯ НЕЙРОСЕТЬЮ ПОД КОНКРЕТНОЕ НАРУШЕНИЕ ======
async function generateComplaintViaAI(channelLink, violationLink, violation) {
    await throttleGemini();

    const prompt = `Напиши короткое официальное письмо-жалобу на английском языке в поддержку Telegram ("Dear Telegram Support Team") о нарушении конкретным каналом Условий использования Telegram.
...
Требования:
- 3 коротких абзаца, формальный деловой тон, без markdown-разметки и списков.
- Естественно упомяни в тексте ссылку на канал и ссылку на пост-доказательство.
- Не упоминай ссылки на законы — они уходят отдельным сообщением.
- Не упоминай название/юзернейм канала как часть описания нарушения — только как ссылку.
- Заверши вежливой финальной фразой.
- После текста будет автоматически добавлена отдельная фраза "Please immediately investigate and take action for violating EU law and Telegram's Terms of Service." — не пиши сама ничего похожего по смыслу, чтобы не получилось повтора.
- Каждый раз формулируй немного иначе, не используй шаблонные фразы дословно.
...
// Вставляет фразу отдельным абзацем прямо перед последним абзацем (обычно
// это финальная вежливая строка вроде "Thank you for..."), а не полагается
// только на то, что нейросеть каждый раз сама её вставит дословно.
function insertBeforeClosing(text, sentence) {
    if (text.includes(sentence)) return text; // AI уже сама вставила — не дублируем
    const idx = text.lastIndexOf('\n\n');
    if (idx === -1) return `${text}\n\n${sentence}`;
    return `${text.slice(0, idx)}\n\n${sentence}${text.slice(idx)}`;
}
...
async function buildComplaint(channelLink, violationLink, violation) {
    ...
    return insertBeforeClosing(text, EFFECTIVE_LINE);
}
...
// ====== ОСНОВНОЙ АНАЛИЗ КАНАЛА ======
    ...
    const textMessages = rawMessages
        ...
        .map((m) => ({ id: m.id, text: m.message, isForwarded: !!m.fwdFrom }));
    ...
    for (let i = 0; i < chunks.length; i++) {
        ...
        try {
            await bot.editMessageText(
                ...
                `📨 Проверено: <b>${done}/${textMessages.length}</b> сообщений\n` +
                `⚠️ Найдено нарушений: <b>${violations.length}</b>\n` +
                `⏱ Прошло времени: ${formatElapsed(Date.now() - scanStartedAt)}`,
                { chat_id: chatId, message_id: progressMsgId, parse_mode: 'HTML' }
            );
        } catch (e) {}
    }

    // прогресс-бар выполнил свою роль — убираем, дальше идут чистые результаты
    ...
    if (!violations.length) {
        ...
        if (failedChunks.length) {
            text += `\n\n⚠️ ${failedChunks.length} из ${chunks.length} пачек сообщений не удалось проверить (нейросеть заблокировала ответ или произошла ошибка) — результат может быть неполным. Причина первой ошибки: ${failedChunks[0].error}`;
        }
    ...
    const genMsg = await bot.sendMessage(chatId, `🤖 <b>Готовлю жалобу</b> под самое серьёзное из ${violations.length} найденных нарушений...`, { parse_mode: 'HTML' });

    // ---- все ссылки на нарушения прямо в чат ----
    let linksText = `🔗 <b>Найденные нарушения — ${violations.length}</b> (по убыванию серьёзности)\n\n`;
    if (failedChunks.length) {
        linksText = `⚠️ <b>${failedChunks.length} из ${chunks.length} пачек не проверились</b> (блокировка нейросетью/ошибка) — возможны ещё нарушения, которые не попали в список.\n\n` + linksText;
    }
    violations.forEach((v, i) => {
        linksText += `${i + 1}. ${channelLink}/${v.id} — <b>${escapeHtml(v.category_ru || 'нарушение')}</b> (${v.severity || '?'}/5)\n   ${escapeHtml(v.reason)}\n\n`;
    });
    for (let i = 0; i < linksText.length; i += 3500) {
        await bot.sendMessage(chatId, linksText.slice(i, i + 3500), { parse_mode: 'HTML' });
    }

    // ---- шаблон жалобы: пишет нейросеть под конкретное нарушение, моноширинный тап-копируемый блок ----
    ...
    await bot.sendMessage(chatId, `📋 <b>Жалоба</b> по самому серьёзному нарушению (${escapeHtml(main.category_ru || '—')}, ${main.severity || '?'}/5) — нажми на текст ниже, чтобы скопировать:`, { parse_mode: 'HTML' });
    await bot.sendMessage(chatId, `<pre>${escapeHtml(complaintText)}</pre>`, { parse_mode: 'HTML' });

    // ---- ссылки на законы — отдельным сообщением, тоже тап-копируемые ----
    const lawLinks = getLawLinks(main.law_type);
    const lawsText = lawLinks.map((l, i) => `${i + 1}. ${l}`).join('\n');
    await bot.sendMessage(chatId, '⚖️ <b>Законы EU</b> по этому типу нарушения — отдельно от жалобы, тоже копируется нажатием:', { parse_mode: 'HTML' });
    await bot.sendMessage(chatId, `<pre>${escapeHtml(lawsText)}</pre>`, { parse_mode: 'HTML' });

    // ---- файл на память со всем отчётом целиком ----
    let report = `Отчёт по каналу: ${channelLink}\nПроверено сообщений: ${textMessages.length}\nНайдено нарушений: ${violations.length}\nВремя проверки: ${formatElapsed(Date.now() - scanStartedAt)}\n\n`;
...
bot.onText(/^\/start$/, (msg) => {
    ...
    const text =
        `🛡 <b>Channel Guard</b> — анализ Telegram-каналов на нарушения\n\n` +
        `Проверяю канал через нейросеть на все виды нарушений правил Telegram и законодательства: слив личных данных, оружие/наркотики, мошенничество, разжигание ненависти и другое.\n\n` +
        `📎 <b>Как пользоваться:</b>\nпросто пришли ссылку на канал, например <code>https://t.me/somechannel</code>\n\n` +
        `Что получишь в ответ:\n` +
        `🔗 все найденные нарушения со ссылками\n` +
        `📋 готовую жалобу под самое серьёзное — нажми на текст, чтобы скопировать\n` +
        `⚖️ ссылки на нужные законы EU отдельным сообщением\n` +
        `📄 полный отчёт файлом\n\n` +
        `⏳ Проверка последних ${SCAN_LIMIT} сообщений занимает от минуты до нескольких.\n` +
        `🔁 Кулдаун между проверками: ${Math.round(SCAN_COOLDOWN_MS / 60000)} мин на пользователя (бот общий, квота нейросети одна на всех).`;
    bot.sendMessage(msg.chat.id, text, {
        parse_mode: 'HTML',
        reply_markup: { inline_keyboard: [[{ text: '📖 Формат ссылки', callback_data: 'help_format' }]] }
    });
});

bot.on('callback_query', async (query) => {
    await bot.answerCallbackQuery(query.id);
    if (query.data === 'help_format') {
        await bot.sendMessage(query.message.chat.id, 'Подходят оба варианта:\n<code>https://t.me/channelname</code>\n<code>@channelname</code>', { parse_mode: 'HTML' });
    }
});
...
bot.on('message', async (msg) => {
    ...
    const wait = await bot.sendMessage(chatId, '🔌 <b>Подключаюсь к каналу...</b>', { parse_mode: 'HTML' });
    try {
