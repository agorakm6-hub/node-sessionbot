...
// ====== УНИВЕРСАЛЬНЫЙ ШАБЛОН ЖАЛОБЫ (БЕЗ НАЗВАНИЯ КАНАЛА) ======
function buildComplaint(channelLink, violationLink) {
    return `Dear Telegram Support Team,

I am writing to report a serious violation of Telegram's Terms of Service. The channel at the following link is engaged in publishing content that violates Telegram's rules and applicable law:

${channelLink}

One example of the violating content can be found at:
${violationLink}

This activity is in direct breach of Telegram's Terms of Service and also infringes upon European legislation regarding data protection and illegal content distribution.

I respectfully request that you investigate this matter and take appropriate action against the channel in question.

Thank you for your attention to this serious issue.`;
}

// ====== ОТДЕЛЬНЫЙ БЛОК С ЗАКОНАМИ ======
function buildLawsBlock() {
    return `📜 Relevant legal provisions referenced in this complaint:\n\n${LAW_LINKS.map((l, i) => `${i + 1}. ${l}`).join('\n')}`;
}
...
async function scanChannel(chatId, username, progressMsgId) {
    ...
    await bot.editMessageText(`⚠️ Найдено ${violations.length} нарушений в ${channelLink}. Присылаю ссылки, шаблон жалобы и список законов.`, { chat_id: chatId, message_id: progressMsgId });
    ...
    // ---- ШАБЛОН ЖАЛОБЫ (моноширинный, без названия канала) ----
    const complaintText = buildComplaint(channelLink, mainLink);
    await bot.sendMessage(chatId, `📋 Шаблон жалобы (нажми на текст ниже, чтобы скопировать):`);
    await bot.sendMessage(chatId, `<pre>${escapeHtml(complaintText)}</pre>`, { parse_mode: 'HTML' });

    // ---- ЗАКОНЫ ОТДЕЛЬНЫМ СООБЩЕНИЕМ ----
    const lawsText = buildLawsBlock();
