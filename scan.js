...
// ====== АНАЛИЗ ПАЧКИ СООБЩЕНИЙ (ПОЛНЫЙ ПРОМПТ) ======
async function analyzeChunk(messages) {
    await throttleGemini();

    const listText = messages.map((m) => `[${m.id}] ${m.text.replace(/\n/g, ' ').slice(0, 500)}`).join('\n');
    const prompt = `Ты — модератор, проверяющий сообщения Telegram-канала на любые нарушения Условий использования Telegram и применимого законодательства.

Ты ДОЛЖЕН найти ВСЕ типы нарушений, включая:

1. ПУБЛИКАЦИЯ ЛИЧНЫХ ДАННЫХ (doxing, data leak):
   - ФИО + номер телефона
   - Паспортные данные
   - СНИЛС, ИНН
   - Адреса проживания
   - Номера автомобилей
   - Любая другая личная информация, опубликованная без согласия

2. НЕЛЕГАЛЬНЫЕ ТОВАРЫ И УСЛУГИ:
   - Продажа оружия
   - Продажа наркотиков
   - Продажа поддельных документов
   - Продажа фальшивых денег
   - Продажа краденых данных (базы, аккаунты)

3. МОШЕННИЧЕСТВО (scam, fraud):
   - Финансовые пирамиды
   - Фейковые инвестиционные схемы
   - Обманные розыгрыши
   - Кража денег под любым предлогом

4. КОНТЕНТ С НЕСОВЕРШЕННОЛЕТНИМИ (CSAM):
   - Сексуализация несовершеннолетних
   - Распространение материалов с детьми

5. ТЕРРОРИЗМ И НАСИЛИЕ:
   - Пропаганда терроризма
   - Призывы к насилию
   - Экстремистские материалы

6. РАЗЖИГАНИЕ НЕНАВИСТИ (hate speech):
   - По признаку расы
   - По признаку религии
...
7. НАРУШЕНИЕ АВТОРСКИХ ПРАВ:
   - Массовое распространение пиратского контента
   - Продажа взломанного ПО
...
9. ЛЮБЫЕ ДРУГИЕ НАРУШЕНИЯ правил Telegram и законов
...
// ====== ГЕНЕРАЦИЯ ШАБЛОНА ЖАЛОБЫ И ЗАКОНОВ ======
...
1. Тему письма (Subject) — кратко и официально, на английском, без русских слов. Например: "Report of Personal Data Leak on Telegram Channel" или "Report of Illegal Goods Distribution on Telegram Channel".

2. Шаблон жалобы в поддержку Telegram на английском языке. Жалоба должна быть официальной, вежливой. ВАЖНЫЕ ПРАВИЛА:
   - НЕ добавляй "[Your Name/Organization]" или любые подписи в конце
   - НЕ используй русские слова типа "probiv", "пробив" — только английские термины: "doxing", "personal data leak", "unauthorized access", "distribution of illegal goods"
   - Опиши КОНКРЕТНОЕ нарушение (что именно публикуется в канале)
   - Укажи ссылку на канал (${channelLink})
   - Укажи ссылку на конкретное сообщение с нарушением (${violationLink})
   - Напиши, какие правила Telegram нарушены

3. Список законов Европейского Союза, которые нарушены в данном конкретном случае. Законы должны быть реальными и соответствовать нарушению. Максимум 3 ссылки. Если подходит меньше — укажи только релевантные.

Нарушение: ${violation.category_ru} (${violation.reason})

Верни ТОЛЬКО JSON-объект (без markdown, без пояснений) вида:
{
  "subject": "тема письма на английском",
  "complaint": "текст жалобы на английском (без Subject в начале, без подписей)",
