...
// Короткое имя, которое ты задал в BotFather при /newapp (поле "Short name").
// Нужно для настоящих t.me-ссылок на конкретный скан, которые открываются
// именно как Mini App внутри Telegram (а не просто как сайт в браузере).
const MINI_APP_SHORT_NAME = process.env.MINI_APP_SHORT_NAME || '';
let BOT_USERNAME = ''; // подтягивается автоматически при старте через bot.getMe()
...
const jobs = new Map();
setInterval(() => {
    // Незавершённые/ошибочные задачи чистим через час — мусор.
    // Завершённые (status === 'done') держим 7 дней — на них могут ссылаться расшаренные ссылки.
    const now = Date.now();
    for (const [id, job] of jobs.entries()) {
        const ttl = job.status === 'done' ? 7 * 24 * 3600000 : 3600000;
    ...
    try {
        if (pathname === '/api/me' && req.method === 'POST') {
            ...
            return sendJson(200, {
                ...
                miniAppShortName: MINI_APP_SHORT_NAME || null
            });
        }
        ...
        if (pathname === '/api/scan/status' && req.method === 'GET') {
            ...
            return sendJson(200, {
                status: job.status,
                statusNote: job.statusNote || null,
        ...
        if (pathname === '/api/report' && req.method === 'POST') {
            ...
            if (!user) return sendJson(401, { error: 'invalid_init_data' });
            const job = jobs.get(body.jobId);
            // Ownership намеренно не проверяем — сканом можно поделиться, и любой,
            // кому прислали ссылку, должен получить репорт по нему (см. /api/scan/share).
            if (!job || !job.result) return sendJson(404, { error: 'not_found' });
            const indices = Array.isArray(body.indices) ? body.indices.slice(0, 15) : []; // разумный потолок за один запрос
            // Раньше репорт+законы для каждого нарушения считались строго по очереди —
            // при нескольких выбранных нарушениях ожидание складывалось. Теперь параллельно
            // (каждый Gemini-запрос сам ротирует ключ и троттлится независимо).
            const results = await Promise.all(indices.map(async (idx) => {
                ...
                const [reportText, lawLinks] = await Promise.all([
                    buildReport(job.result.channelLink, vLink, v),
                    getLawLinksForViolation(v)
                ]);
                return { index: idx, link: vLink, category: v.category_ru, severity: v.severity, reportText, lawLinks };
            }));
            const out = results.filter(Boolean).sort((a, b) => a.index - b.index);
...
server.listen(PORT, async () => {
    ...
    try {
        ...
        console.log(`✅ Юзернейм бота: @${BOT_USERNAME}`);
        if (!MINI_APP_SHORT_NAME) console.warn('⚠️ MINI_APP_SHORT_NAME не задан — ссылки "Поделиться" будут вести на обычный сайт, а не открываться как Mini App внутри Telegram.');
...
${listText}`;
    ...
    try {
        res = await fetch(geminiUrlFor(key), {
            ...
            signal: AbortSignal.timeout(25000), // без этого зависший запрос ждали бы бесконечно
            body: JSON.stringify({
...
- Без пояснений и вступлений, только список.`;
    ...
    const res = await fetch(geminiUrlFor(key), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        signal: AbortSignal.timeout(25000), // google_search grounding — реальный веб-поиск, чуть щедрее таймаут
        body: JSON.stringify({
            contents: [{ parts: [{ text: prompt }] }],
            tools: [{ google_search: {} }],
...
async function getAdminAuthoredMessages(client, entity, adminIds, generalMessages) {
    ...
    for (const adminId of adminIds) {
        try {
            const msgs = await withTimeout(client.getMessages(entity, { limit: perAdminLimit, fromUser: adminId }), 15000, `сообщения админа ${adminId}`);
...
// Оборачивает промис таймаутом — без этого зависший MTProto-запрос (не Gemini,
// а сам Telegram) мог бы висеть бесконечно и выглядеть как "0% и ничего не происходит".
function withTimeout(promise, ms, label) {
    return Promise.race([
        promise,
        new Promise((_, reject) => setTimeout(() => reject(new Error(`Таймаут: ${label} не ответил за ${Math.round(ms / 1000)}с`)), ms))
    ]);
}
...
async function runScan(link, mode, { onStatus, onProgress, abortFlag } = {}) {
    ...
    if (link.type === 'invite') {
        ...
        entity = await withTimeout(client.getEntity(link.username), 20000, 'получение канала');
    }
    ...
    const rawMessages = await withTimeout(client.getMessages(entity, { limit: SCAN_LIMIT }), 30000, 'получение сообщений');
    ...
    // Сразу сообщаем total, ещё до первого завершённого чанка — иначе при
    // маленьком канале (1 чанк) фронт видит "0 из 0" и выглядит зависшим,
    // хотя на деле просто ждёт единственный запрос к Gemini.
    if (onProgress) await onProgress({ done: 0, total: textMessages.length, violCount: 0 });
