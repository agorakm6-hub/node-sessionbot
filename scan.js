...
${listText}`;
    ...
    if (data.promptFeedback?.blockReason) throw new Error(`Gemini заблокировал пачку (${data.promptFeedback.blockReason}) — проверь вручную.`);
    const candidate = data.candidates?.[0];
    if (candidate?.finishReason === 'SAFETY' || candidate?.finishReason === 'PROHIBITED_CONTENT') throw new Error(`Gemini заблокировал ответ (${candidate.finishReason}) — проверь вручную.`);
