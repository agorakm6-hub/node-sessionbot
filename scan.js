...
async function analyzeChunk(messages, tosText, attempt = 1) {
    ...
    const prompt = `Ты — эксперт по модерации контента. Твоя задача — тщательно проанализировать сообщение из Telegram-канала или группы на предмет нарушения:
...
${tosText || 'Стандартные правила Telegram: запрещён незаконный контент, спам, угрозы, доксинг, CSAM, насилие, мошенничество, разжигание ненависти, нарушение авторских прав.'}
=== конец условий ===

Проверь сообщение по этим категориям:
1. **CSAM** — контент с сексуализацией несовершеннолетних (САМЫЙ СЕРЬЁЗНЫЙ)
2. **Личные данные** — ФИО + телефон/паспорт/СНИЛС/ИНН/адрес без согласия (доксинг)
3. **Терроризм и насилие** — пропаганда терроризма, угрозы, призывы к насилию
4. **Незаконные товары** — оружие, наркотики, поддельные документы
5. **Разжигание ненависти** — по рассе, религии, национальности, полу
6. **Мошенничество** — финансовые схемы, фишинг, вредоносное ПО
7. **Авторские права** — массовое нарушение
8. **Другие нарушения** — иные нарушения ToS

⚠️ Если сообщение [ПЕРЕСЛАНО] — это репост. Оценивай сам контент.

Верни ТОЛЬКО JSON-массив (без markdown):
[{"id":123,"category_ru":"тип нарушения","category_en":"short EN phrase","law_type":"personal_data|illegal_goods|csam|terrorism_violence|hate_speech|copyright|malware_fraud|other","severity":1-5,"reason":"причина"}]

severity: 5=бан, 4=вероятный бан, 3=нарушение ToS, 2=подозрительно, 1=незначительно
...
${listText}`;
    ...
    try {
        ...
        if (!res.ok) {
            ...
            if ((res.status === 503 || res.status === 500 || res.status === 429) && attempt < MAX_ATTEMPTS) {
        ...
        if (blockReason || finishReason === 'SAFETY' || finishReason === 'PROHIBITED_CONTENT') {
            if (messages.length === 1) {
                return [{
                    id: messages[0].id,
                    category_ru: '🔴 ЗАБЛОКИРОВАНО — нарушение максимальной тяжести',
                    category_en: 'Critical violation - content blocked by AI safety filter',
                    law_type: 'other',
                    severity: 5,
                    reason: `Контент заблокирован системой безопасности Gemini (${blockReason || finishReason}).`
                }];
            }
            const mid = Math.ceil(messages.length / 2);
            const left = await analyzeChunk(messages.slice(0, mid), tosText, 1).catch(() => []);
            const right = await analyzeChunk(messages.slice(mid), tosText, 1).catch(() => []);
            return [...left, ...right];
        }
...
async function scanChannel(chatId, username, progressMsgId, selectedMode) {
    ...
    if (!textMessages.length) {
        await bot.editMessageText(
            isGroupChat ? '⚠️ Не нашлось сообщений от админов группы.' : '⚠️ В канале нет текстовых сообщений.',
            { chat_id: chatId, message_id: progressMsgId, parse_mode: 'HTML' }
        );
        return;
    }
    ...
    let linksText = `🔗 <b>Найденные нарушения — ${violations.length}</b>\n\n`;
    ...
    let complaintText = await buildComplaint(channelLink, mainLink, main);
    try { await bot.deleteMessage(chatId, genMsg.message_id); } catch (e) {}

    await bot.sendMessage(chatId, `📋 <b>Жалоба</b>\n👇 Нажми на текст, чтобы скопировать:`, { parse_mode: 'HTML' });
...
function getTutorialText() {
    return `📋 <b>Как отправить жалобу на DSA</b>\n\n` +
           `<b>Шаг 1:</b> <a href="https://telegram.org/dsa-report">https://telegram.org/dsa-report</a>\n` +
           `⚠️ Используйте VPN с сервером в <b>Нидерландах</b>\n\n` +
           `<b>Шаг 2:</b> Зарегистрируйтесь\n` +
           `<b>Шаг 3:</b> Вставьте ссылку на канал/группу\n` +
           `<b>Шаг 4:</b> Вставьте ссылку на нарушение\n` +
           `<b>Шаг 5:</b> Выберите причину жалобы\n` +
           `<b>Шаг 6:</b> Вставьте текст жалобы от бота\n` +
           `<b>Шаг 7:</b> Выберите страну (EU law)\n` +
           `<b>Шаг 8:</b> Вставьте ссылки на законы EU\n` +
           `<b>Шаг 9:</b> Пропустите документацию\n` +
...
function startMenuText() {
    return `🛡 <b>Channel Guard</b> — анализ каналов и групп\n\n` +
           `🔍 Проверяю нарушения:\n• ToS Telegram\n• EU law\n\n` +
           `📋 Что получишь:\n🔗 ссылки на нарушения\n📋 жалобу\n⚖️ законы EU\n📄 отчёт файлом\n\n` +
           `⏳ Проверка ${SCAN_LIMIT} сообщений: 1-5 мин\n` +
           `🔁 Кулдаун: ${Math.round(SCAN_COOLDOWN_MS / 60000)} мин\n\n` +
           `📌 <b>Выберите тип:</b>`;
}
...
function startMenuKeyboard() {
    return {
        inline_keyboard: [
            ...
            [{ text: '📖 Как отправить жалобу', callback_data: 'tutorial' }],
...
bot.on('callback_query', async (query) => {
    ...
    if (data === 'tutorial') {
        await bot.editMessageText(getTutorialText(), {
            chat_id: chatId,
            message_id: messageId,
            parse_mode: 'HTML',
            reply_markup: { inline_keyboard: [[{ text: '◀️ Назад в меню', callback_data: 'back_to_start' }]] }
        });
        return;
    }
...
bot.on('message', async (msg) => {
    ...
    if (remaining > 0) {
        ...
        await bot.sendMessage(chatId, `⏳ Подожди ещё <b>~${mins} мин</b> перед следующей проверкой.`, { parse_mode: 'HTML' });
        return;
    }
