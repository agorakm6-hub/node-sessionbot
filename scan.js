...
// ====== ПРОГРЕСС-БАР (тонкая полоска из зелёных блоков — Telegram не поддерживает
// плавную заливку цветом в тексте, это лучшая имитация без Mini App) ======
function renderProgressBar(current, total, width = 14) {
    ...
    for (let i = 0; i < chunks.length; i++) {
        ...
        try {
            await bot.editMessageText(
                `🔍 <b>Сканирую канал</b>\n\n${renderProgressBar(done, textMessages.length)}\n\n${done}/${textMessages.length} сообщений · найдено нарушений: ${violations.length}`,
