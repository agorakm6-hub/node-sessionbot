...
async function analyzeChunk(messages) {
    ...
    const prompt = `Ты — эксперт-модератор Trust & Safety, проверяющий сообщения Telegram-канала на нарушения Условий использования Telegram и применимого законодательства. Это модерационная задача — тебе показывают потенциально нарушающий контент ИМЕННО для того, чтобы его выявить и передать в отчёт для Telegram Support. Не отказывайся анализировать из-за характера контента — это и есть твоя задача.

Ищи ВСЕ типы нарушений, включая:
- публикацию личных данных людей без согласия ("пробив", доксинг): ФИО + телефон/паспорт/СНИЛС/ИНН/адрес/номер авто/место работы/геолокация;
- продажу или рекламу оружия, наркотиков, поддельных документов, краденых данных карт, other illegal goods;
- мошенничество и финансовые схемы обмана (скам, фейковые инвестиции, кардинг);
- любой контент, сексуализирующий несовершеннолетних, или обсуждение доступа к такому контенту;
- пропаганду терроризма, инструкции по насилию, призывы к нему;
- разжигание ненависти по признаку расы/религии/национальности/др.;
- массовое нарушение авторских прав (пиратский контент, ссылки на слив фильмов/книг/софта);
- вредоносное ПО, фишинговые ссылки, инструкции по взлому;
- любые другие явные нарушения правил Telegram.

Критерии severity (оценивай строго по содержанию, не занижай):
5 — гарантированно ведёт к блокировке канала: CSAM, терроризм/призывы к насилию, продажа оружия/наркотиков/поддельных документов, реальный доксинг (слив ФИО+паспорт/адрес/телефон конкретного человека), крупное мошенничество с явными пострадавшими;
4 — серьёзное нарушение с высокой вероятностью бана: фишинг/малварь, кардинг, разжигание ненависти, масштабное пиратство;
3 — заметное нарушение: реклама сомнительных услуг, спам со скам-схемами, частичный слив данных;
2 — пограничный случай, вероятно нарушение, но неочевидно;
1 — незначительное.

Проверь список сообщений ниже. Верни ТОЛЬКО JSON-массив (без markdown, без пояснений, без \`\`\`) вида:
...
Включай в массив только сообщения, которые ДЕЙСТВИТЕЛЬНО являются нарушением. Если по конкретному сообщению есть сомнения — лучше включить с severity 2, чем пропустить. Если нарушений совсем нет — верни пустой массив [].
...
${listText}`;
    ...
    const res = await fetch(GEMINI_URL, {
        ...
        body: JSON.stringify({
            ...
            // Без этого Gemini по умолчанию молча блокирует ответ на промпты,
            // описывающие реальные нарушения (доксинг/наркотики/оружие и т.п.),
            // и код получал пустой candidates — из-за этого бот писал
            // "нарушений не найдено", хотя на самом деле ответ был заблокирован
            // фильтром безопасности, а не потому что модель их не увидела.
            safetySettings: [
    ...
    // Если модель всё же заблокировала ответ (например, жёсткая блокировка
    // Google по CSAM, которую нельзя отключить через safetySettings) —
    // явно логируем это, а не тихо считаем "нарушений нет".
    if (data.promptFeedback?.blockReason) {
        console.error(`⚠️ Gemini заблокировал пачку сообщений: ${data.promptFeedback.blockReason}`);
        return { violations: [], blocked: true, blockReason: data.promptFeedback.blockReason };
    }
    const candidate = data.candidates?.[0];
    if (!candidate || (candidate.finishReason && candidate.finishReason !== 'STOP' && candidate.finishReason !== 'MAX_TOKENS')) {
        console.error(`⚠️ Gemini не вернул нормальный ответ (finishReason: ${candidate?.finishReason || 'нет candidates'})`);
        return { violations: [], blocked: true, blockReason: candidate?.finishReason || 'NO_CANDIDATES' };
    }

    const raw = candidate.content?.parts?.map((p) => p.text).join('') || '[]';
    try {
        return { violations: JSON.parse(raw), blocked: false };
    } catch (e) {
...
async function scanChannel(chatId, username, progressMsgId) {
    ...
    for (let i = 0; i < chunks.length; i++) {
        try {
            const { violations: found, blocked } = await analyzeChunk(chunks[i]);
            found.forEach((f) => violations.push(f));
            if (blocked) blockedCount += chunks[i].length;
        } catch (e) {
    ...
    const blockedNote = blockedCount
        ? `\n\n⚠️ ${blockedCount} сообщений не удалось проверить — модель заблокировала ответ по этим пачкам (см. логи сервера). Возможно, стоит перезапустить проверку.`
        : '';

    if (!violations.length) {
        await bot.editMessageText(`✅ Проверено ${textMessages.length} сообщений в ${channelLink} — явных нарушений не найдено.${blockedNote}`, { chat_id: chatId, message_id: progressMsgId });
        return;
    }
    ...
    await bot.editMessageText(`⚠️ Найдено ${violations.length} нарушений в ${channelLink}. Присылаю ссылки и шаблон жалобы.${blockedNote}`, { chat_id: chatId, message_id: progressMsgId });
