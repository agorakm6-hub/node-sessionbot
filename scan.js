...
const CHUNK_SIZE = 10; // ИЗМЕНЕНО С 1 НА 10
...
// ====== ТВОЙ ОРИГИНАЛЬНЫЙ АНАЛИЗ (НЕ ТРОГАЛ) ======
async function analyzeChunk(messages, tosText, attempt = 1) {
    ...
    const prompt = `Ты — тщательный модератор, разбирающий ОДНО сообщение Telegram-канала/группы построчно на нарушения официальных Условий использования Telegram (текст ниже) и применимого законодательства EU. Не спеши с выводом "нарушений нет" — сначала обдумай сообщение по каждому пункту ниже, включая:
...
async function scanChannel(chatId, username, progressMsgId, mode) {
    ...
    if (entity.className === 'User') {
        await bot.editMessageText('❌ Это ссылка на профиль пользователя, а не на канал или группу.', { chat_id: chatId, message_id: progressMsgId });
        return;
    }
    ...
    if (isGroupChat) {
        try {
            await bot.editMessageText('👥 Определяю админов...', { chat_id: chatId, message_id: progressMsgId });
        } catch (e) {}
    ...
    if (!textMessages.length) {
        await bot.editMessageText(
            isGroupChat ? '⚠️ Нет сообщений от админов.' : '⚠️ В канале нет текстовых сообщений.',
    ...
    const genMsg = await bot.sendMessage(chatId, `🤖 <b>Готовлю жалобу</b> (${violations.length} нарушений)...`, { parse_mode: 'HTML' });

    let linksText = `🔗 <b>Нарушения — ${violations.length}</b>\n\n`;
...
function startMenuText() {
    return `🛡 <b>Channel Guard</b> — анализ каналов и групп\n\n` +
           `🔍 Проверяю нарушения ToS Telegram и EU law\n\n` +
           `📋 Что получишь:\n🔗 ссылки на нарушения\n📋 жалобу\n⚖️ законы EU\n📄 отчёт файлом\n\n` +
           `⏳ Проверка ${SCAN_LIMIT} сообщений\n` +
...
function startMenuKeyboard() {
    return { inline_keyboard: [
        ...
        [{ text: '📖 Как отправить жалобу на DSA', callback_data: 'dsa_tutorial' }],
...
bot.on('callback_query', async (query) => {
    ...
    if (data === 'dsa_tutorial') {
        await bot.editMessageText(getDsaTutorial(), {
            chat_id: chatId,
...
bot.on('message', async (msg) => {
    ...
    if (remaining > 0) {
        ...
        await bot.sendMessage(chatId, `⏳ Подожди ~${mins} мин`, { parse_mode: 'HTML' });
        return;
    }
