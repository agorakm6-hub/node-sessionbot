...
// ====== ШАБЛОН ЖАЛОБЫ ======
function buildComplaint(channelLink, violationLink, category) {
    const desc = category ? category.toLowerCase() : 'публикация запрещённого контента';
    return `Dear Telegram Support Team,

I am writing to report a serious violation of Telegram's Terms of Service. The channel ${channelLink} is engaged in ${desc}, as evidenced by the post ${violationLink}. This activity not only violates Telegram's policies but also infringes upon European law.

This behavior is unacceptable and requires immediate attention. I urge you to investigate this matter promptly and take appropriate action to address the violation.

This complaint is submitted with reference to the following provisions of European law:
...
async function scanChannel(chatId, username, progressMsgId) {
    ...
    violations.forEach((v, i) => {
        linksText += `${i + 1}. ${channelLink}/${v.id} — ${v.category || 'нарушение'} (${v.severity || '?'}/5)\n   ${v.reason}\n\n`;
    });
    ...
    const complaintText = buildComplaint(channelLink, mainLink, main.category);
    await bot.sendMessage(chatId, `📋 Шаблон жалобы по самому серьёзному нарушению (${main.category || '—'}, ${main.severity || '?'}/5) — нажми на текст ниже, чтобы скопировать:`);
    ...
    violations.forEach((v) => { report += `${channelLink}/${v.id} — [${v.category || '—'}, severity ${v.severity || '?'}/5] ${v.reason}\n`; });
