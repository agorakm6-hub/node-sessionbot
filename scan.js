...
async function scanChannel(chatId, username, progressMsgId) {
    ...
    await bot.editMessageText(`⚠️ Найдено ${violations.length} нарушений в ${channelLink}. Готовлю жалобу под конкретный случай...`, { chat_id: chatId, message_id: progressMsgId });
    ...
    await bot.sendMessage(chatId, `📋 Жалоба по самому серьёзному нарушению (${main.category_ru || '—'}, ${main.severity || '?'}/5) — нажми на текст ниже, чтобы скопировать:`);
    ...
    await bot.sendMessage(chatId, '⚖️ Ссылки на законы EU по этому типу нарушения (отдельно от шаблона, тоже можно скопировать нажатием):');
