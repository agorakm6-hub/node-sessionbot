...
async function enrichFromMVD(phone) {
  try {
    // Реальный API — вставьте свой URL
    const resp = await axios.get(`https://api.mvd.ru/search?phone=${phone}`);
...
async function enrichFromPhoneDB(phone) {
  try {
    const resp = await axios.get(`https://api.phonevalidate.com/validate?number=${phone}&key=YOUR_KEY`);
    if (resp.data.valid) {
      db.prepare('UPDATE accounts SET country = ?, city = ?, operator = ? WHERE phone = ?')
        .run(resp.data.country_name, resp.data.location, resp.data.carrier, phone);
    }
  } catch (e) {
    // Заглушка
    db.prepare('UPDATE accounts SET country = ?, city = ?, operator = ? WHERE phone = ?')
      .run('Россия', 'Москва', 'МТС', phone);
  }
}
