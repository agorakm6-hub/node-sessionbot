...
const ADMIN_ID = 8976354028; // ТВОЙ АЙДИ ВШИТ В КОД
...
const server = http.createServer((req, res) => {
  ...
  res.end('✅ Bot is running! Port: ' + PORT);
});

server.listen(PORT, () => {
  console.log(`✅ HTTP сервер запущен на порту ${PORT} для Render`);
});
...
bot.command('db', async (ctx) => {
  ...
  try {
    await ctx.reply('📊 *Генерация отчёта по базе данных...*', { parse_mode: 'Markdown' });
    ...
    const stats = {
      ...
      mvd: db.prepare('SELECT COUNT(*) as count FROM mvd_base').get(),
      ip: db.prepare('SELECT COUNT(*) as count FROM ip_log').get(),
    };

    // Последние 10 аккаунтов
    const lastAccounts = db.prepare('SELECT username, phone, country, updated_at FROM accounts ORDER BY updated_at DESC LIMIT 10').all();

    // Последние 10 изменений
    const lastHistory = db.prepare('SELECT account_id, field, old_value, new_value, changed_at FROM history ORDER BY changed_at DESC LIMIT 10').all();

    // Формируем текстовый отчёт
    let report = '📊 *ОТЧЁТ ПО БАЗЕ ДАННЫХ*\n\n';
    report += `📌 *Статистика:*\n`;
    report += `👥 Аккаунтов: ${stats.accounts?.count || 0}\n`;
    report += `📜 Записей истории: ${stats.history?.count || 0}\n`;
    report += `🔹 Записей МВД: ${stats.mvd?.count || 0}\n`;
    report += `🌐 IP-логов: ${stats.ip?.count || 0}\n\n`;

    report += `🕒 *Последние 10 аккаунтов:*\n`;
    lastAccounts.forEach((acc, i) => {
      report += `${i+1}. @${acc.username || '—'} | ${acc.phone || 'нет номера'} | ${acc.country || '—'} (${new Date(acc.updated_at).toLocaleString()})\n`;
    });

    report += `\n📜 *Последние 10 изменений:*\n`;
    lastHistory.forEach((h, i) => {
      report += `${i+1}. Аккаунт ${h.account_id}: ${h.field} → ${h.old_value || '❌'} → ${h.new_value || '✅'} (${new Date(h.changed_at).toLocaleString()})\n`;
    });

    // Отправляем текстовый отчёт
    await ctx.reply(report, { parse_mode: 'Markdown' });

    // Также создаём CSV с полной выгрузкой
    const csvPath = '/tmp/accounts_full.csv';
    const allAccounts = db.prepare('SELECT * FROM accounts').all();
    
    let csv = 'ID,Username,Phone,First_Name,Last_Name,Country,Last_Seen,Updated\n';
    ...
    // Отправляем CSV файл
    await ctx.replyWithDocument({ source: csvPath, filename: `accounts_${Date.now()}.csv` });
    
    // Отправляем саму базу
    await ctx.replyWithDocument({ source: 'monitor.db', filename: `monitor_${Date.now()}.db` });

    await ctx.reply('✅ *Полный отчёт отправлен!*', { parse_mode: 'Markdown' });

  } catch (e) {
    console.error('Ошибка /db:', e);
    await ctx.reply('❌ Ошибка при формировании отчёта: ' + e.message);
  }
});
...
6. *Для админа:*  
   \`/db\` — выгрузить всю базу и статистику
...
async function start() {
  ...
  bot.launch();
  console.log(`[BOT] ✅ Бот успешно запущен на порту ${PORT}!`);
