...
// ======== СНАЧАЛА ПОДНИМАЕМ ПОРТ (чтобы Render не ругался) ========
...
// ======== ПЕРЕСОЗДАЁМ БД (удаляем старую с ошибками) ========
try {
  if (fs.existsSync('monitor.db')) {
    fs.unlinkSync('monitor.db');
    console.log('🗑️ Старая БД удалена');
  }
} catch(e) {}

// ======== СОЗДАЁМ НОВУЮ БД С ПРАВИЛЬНЫМИ ТИПАМИ ========
...
`, (err) => {
  ...
  else console.log('✅ Новая БД создана с правильными типами');
});
...
async function monitorAll() {
  ...
  db.all('SELECT id, username, phone FROM accounts', [], async (err, accounts) => {
    ...
    // Расширение
    console.log('[MONITOR] Расширение...');
    try {
      if (clients.length > 0) {
...
bot.command('db', async (ctx) => {
  ...
  try {
    ...
    const allAccounts = db.prepare('SELECT * FROM accounts LIMIT 50').all();
    
    let report = '📊 *ОТЧЁТ ПО БАЗЕ*\n\n';
    report += `👥 Аккаунтов: ${stats.accounts?.count || 0}\n`;
    report += `📜 Истории: ${stats.history?.count || 0}\n\n`;
    report += `📌 *Последние 20 аккаунтов:*\n`;
    allAccounts.forEach(acc => {
      report += `@${acc.username || acc.id} | ${acc.phone || 'нет номера'} | ${acc.country || '—'}\n`;
    });
...
bot.start(async (ctx) => {
  await ctx.reply(`
🔍 *БОТ-ПРОБИВ + МОНИТОРИНГ*

/search @username — поиск по юзернейму
/phone +номер — поиск по номеру (все страны)
/ip 1.2.3.4 — поиск по IP
/db — выгрузка БД (только админ)
