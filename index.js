...
async function initUserBot() {
  ...
  try {
    ...
    mtClient = new TelegramClient(stringSession, apiId, apiHash, {
      connectionRetries: -1, // Бесконечные попытки переподключения при разрывах связи
      timeout: 60000,
      useWSS: false,
    });
    ...
    // Автоматическая попытка повторного запуска через 15 секунд при сбое
...
bot.on('message', async (msg) => {
  ...
  if (text.startsWith('@') || !text.includes(' ')) {
    ...
    // Если нет в локальной базе, принудительно запрашиваем у Telegram через глобальный поиск (Userbot)
    if (!foundUser && mtClient) {
      try {
        ...
        if (resolved) {
          ...
          // Повторный поиск после сохранения найденных данных
          for (const key of Object.keys(db.users)) {
            const u = db.users[key];
...
// Фоновый непрерывный мониторинг входящих сообщений во всех отслеживаемых группах через Userbot (24/7)
