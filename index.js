...
const sqlite3 = require('sqlite3'); // ← изменено с better-sqlite3
...
// Инициализация БД (sqlite3 в режиме verbose для детализации)
const db = new sqlite3.Database('monitor.db', sqlite3.OPEN_READWRITE | sqlite3.OPEN_CREATE, (err) => {
  if (err) console.error('Ошибка открытия БД:', err.message);
});

// Создание таблиц (обёрнуто в exec для множественных запросов)
db.exec(`
...
function enrichFromMVD(phone) {
  try {
    // Заглушка — в реальности здесь axios запрос
    ...
    db.run('INSERT OR REPLACE INTO mvd_base (phone, full_name, passport, wanted) VALUES (?,?,?,?)',
      [phone, data.full_name, data.passport, data.wanted ? 1 : 0]);
  } catch (e) {
    console.log('Ошибка МВД:', e.message);
  }
}

// ======== ОБОГАЩЕНИЕ ИЗ ТЕЛЕФОННЫХ БАЗ ========
function enrichFromPhoneDB(phone) {
  try {
    ...
    db.run('UPDATE accounts SET country = ?, operator = ? WHERE phone = ?',
      [country, 'Неизвестно', phone]);
  } catch (e) {
...
async function monitorAll() {
  ...
  db.all('SELECT id, username, phone FROM accounts', [], async (err, accounts) => {
    ...
    if (accounts.length === 0) {
      ...
      // Перезапускаем мониторинг через 5 секунд
      setTimeout(() => monitorAll(), 5000);
      return;
    }
    ...
    for (let acc of accounts) {
      try {
        ...
        db.get('SELECT * FROM accounts WHERE id = ?', [acc.id], (err, old) => {
          if (err) { console.error(err); return; }
          if (old) {
            for (let field of ['phone', 'username', 'first_name', 'last_name', 'bio']) {
              if (old[field] !== newData[field]) {
                db.run('INSERT INTO history (account_id, field, old_value, new_value, changed_at) VALUES (?,?,?,?,?)',
                  [acc.id, field, old[field], newData[field], Date.now()]);
              }
            }
          }
          ...
          db.run(`
            ...
            VALUES (?,?,?,?,?,?,?,?)
          `, [newData.id, newData.username, newData.phone, newData.first_name, newData.last_name, newData.bio, newData.last_seen, Date.now()]);

          if (newData.phone) {
            enrichFromMVD(newData.phone);
            enrichFromPhoneDB(newData.phone);
          }
        });
...
bot.command('search', async (ctx) => {
  ...
  db.get('SELECT * FROM accounts WHERE username LIKE ?', [`%${username}%`], async (err, acc) => {
    if (err) { console.error(err); return ctx.reply('Ошибка БД'); }
    if (!acc) {
      db.run('INSERT OR IGNORE INTO accounts (username) VALUES (?)', [username]);
      return ctx.reply('Добавлен в базу, данные появятся после следующего цикла.');
    }

    db.all('SELECT * FROM history WHERE account_id = ? ORDER BY changed_at DESC LIMIT 15', [acc.id], (err, history) => {
      if (err) { console.error(err); return ctx.reply('Ошибка БД'); }
...
bot.command('phone', async (ctx) => {
  ...
  db.get('SELECT * FROM accounts WHERE phone = ?', [phone], (err, acc) => {
    if (err) { console.error(err); return ctx.reply('Ошибка БД'); }
    db.get('SELECT * FROM mvd_base WHERE phone = ?', [phone], (err, mvd) => {
      let msg = `📞 *Поиск по номеру ${phone}:*\n`;
      if (acc) {
        msg += `🆔 Аккаунт: @${acc.username || acc.id}\n👤 Имя: ${acc.first_name || ''}\n🌍 Страна: ${acc.country || '—'}, Город: ${acc.city || '—'}\n📡 Оператор: ${acc.operator || '—'}\n`;
      } else {
        msg += `❌ Не найден. Добавляю в мониторинг.\n`;
        db.run('INSERT OR IGNORE INTO accounts (phone) VALUES (?)', [phone]);
      }
...
async function start() {
  ...
  // Запускаем мониторинг сразу и затем по расписанию
  monitorAll();
  cron.schedule('*/10 * * * *', () => {
    monitorAll();
  });
